procedure wfpfit (photfile, apert)

file    photfile 	{prompt="File containing photometry ?"}
file	eqfile		{prompt="Output log file? \n", mode="q"}
real	apert=INDEF	{prompt="Aperture to use or INDEF to choose aperture", mode="q"}
int	ccd		{prompt="CCD to calibrate ? (0 to mix all CCDs)", mode="q"}

string	band		{prompt="Band ?", mode="q", enum="U|B|V|R|I"}
...
real	_ku=0.50		{prompt="U band atmospheric extinction coefficient ?"}
real	_kb=0.23		{prompt="B band atmospheric extinction coefficient ?"}
real	_kv=0.12		{prompt="V band atmospheric extinction coefficient ?"}
real	_kr=0.09		{prompt="R band atmospheric extinction coefficient ?"}
real	_ki=0.02		{prompt="I band atmospheric extinction coefficient ?"}
...
file	ban, ref, stand, outfile, fit, outtbl, fields, act_phot, _now, colu, _stdcat
...
del(stand, ve-, >>&errors)
fd1=_stdcat
while(fscan(fd1, str1, str2, alp, dec1, magv, magbv, magub, magvr,
		magri, magvi)!=EOF) {
	if(str1!=field) next
...
_ccd=ccd
#-EVH:
if (_ccd==0) print("Your choice is to mix ALL CCDs ")
else print("Your choice is CCD ", _ccd)
