...
string		apertures="20,25,30"	 {prompt="List of aperture radii in pixels"}
real		annulus=35.      {prompt="Inner radius of sky annulus in pixels"}

pset		photpars 	 {prompt="Other Photometry  parameters (-> :e) ?"}
pset		centerpars 	 {prompt="Other Centering   parameters (-> :e) ?"}
pset            fitskypars 	 {prompt="Other Sky fitting parameters (-> :e) ?"}
string		instr = "wfi2p2" {prompt="Instrument file ? (set by WFPSETUP)"}
...
#-EVH:				tvmark(1,coords=stars,mark="circle", radii=40, color=213,label+, nxoff=10,nyoffs=10)
			   tvmark(1,coords=stars,mark="circle", radii=40,color=213,txsiz=7,label+, nxoff=10,nyoffs=10)
			}
		if (first) {
			first=no
			}
...
#-EVH: to force zmag=25 in photpars ... because APPHOT messes up 
#      the output if zmag is 0.0
		photpars.zmag = zeropoint
#-EVH added pars:
		photpars.apertures = _apertu
		fitskypars.annulus = _annulu
#- to be sure that the EXP.TIME keyword is taken into account: 
